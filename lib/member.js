        ...
        return new Promise(async (resolve) => {
            await axios.post(`${this.API().url}/member/`, JSON.stringify(member.content), {
                ...
                .then(async (res) => {
                    ...
                    resolve(await system.findMemberById(res.data))
                })
                .catch(err => console.error('<Member>.create() method threw an error: ' + err.toJSON().message))
        })
    }
    ...
    delete = async () => {
        ...
        return new Promise(async (resolve) => {
            await axios.delete(`${this.API().url}/member/${member.id}`, {
                ...
                .then(async () => {
                    let system = new System(this.API())
                    await system.findMemberById(member.id)
                    .then((match) => {
