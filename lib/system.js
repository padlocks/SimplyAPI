...
class System extends SAPI {
    ...
    // Allowed once every 24h, does not seem to return the exported data.
    // export = async () => {
    //     return new Promise(async (resolve) => {
    //         let exported = await axios.post(`${this.API.url}/user/${this.API.userId}/export`, {}, {
    //             headers: this.API.header
    //         })
    //             .catch(err => console.error(`<System>.export() method threw an error: ${err.message}\nReason: ${err.response?.data}`))
    ...
    getAnalytics = async () => {
        return new Promise(async (resolve) => {
            let { data } = await axios.get(`${this.API.url}/user/analytics`, {
                headers: this.API.header
            })
