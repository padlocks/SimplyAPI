...
class Comment extends SAPI {
    ...
    create = async () => {
        ...
        return new Promise(async (resolve) => {
            ...
            .catch(err => console.error(`<Comment>.create() method threw an error: ${err.message}\nReason: ${err.response?.data}`))
        })
    }
    ...
    update = async () => {
        ...
        return new Promise(async (resolve) => {
            await axios.patch(`${this.API().url}/comment/${comment.id}`, JSON.stringify({ text: comment.content.text }), {
                ...
                .catch(err => console.error(`<Comment>.update() method threw an error: ${err.message}\nReason: ${err.response?.data}`))
        })
    }
    ...
    delete = async () => {
        ...
        return new Promise(async (resolve) => {
            await axios.delete(`${this.API().url}/comment/${comment.id}`, {
                ...
                .catch(err => console.error(`<Comment>.delete() method threw an error: ${err.message}\nReason: ${err.response?.data}`))
        })
    }
}
