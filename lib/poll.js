    ...
    create = async () => {
        ...
        return new Promise(async (resolve) => {
            await axios.post(`${this.API().url}/poll`, JSON.stringify(poll.content), {
                ...
                .catch(err => console.error(`<Poll>.create() method threw an error: ${err.message}\nReason: ${err.response?.data}`))
        })
    }
    ...
    vote = async (memberId, comment, choice) => {
        ...
        return new Promise(async (resolve) => {
            await axios.patch(`${this.API().url}/poll/${this.id}`, JSON.stringify(vote), {
                ...
                .catch(err => console.error(`<Poll>.vote() method threw an error: ${err.message}\nReason: ${err.response?.data}`))
        })
    }
    ...
    update = async () => {
        ...
        return new Promise(async (resolve) => {
            await axios.patch(`${this.API().url}/poll/${poll.id}`, poll.content, {
                ...
                .catch(err => console.error(`<Poll>.update() method threw an error: ${err.message}\nReason: ${err.response?.data}`))
        })
    }
    ...
    delete = async () => {
        ...
        return new Promise(async (resolve) => {
            await axios.delete(`${this.API().url}/poll/${poll.id}`, {
                ...
                .catch(err => console.error(`<Poll>.delete() method threw an error: ${err.message}\nReason: ${err.response?.data}`))
        })
    }
}
